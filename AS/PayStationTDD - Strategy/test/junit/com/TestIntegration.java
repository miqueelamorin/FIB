...
public class TestIntegration {
  ...
  public void shouldIntegrateLinearRateCorrectly() 
    ...
    ps = new PayStationImpl( new LinearRateStrategy() , new Revised(new Date().toString()) );
    // add $ 2.0: 
    addOneDollar(); addOneDollar();
    
    assertEquals( "Linear Rate: 2$ should give 80 min ",
                  80 , ps.readDisplay() );
    assertEquals("Should be in revised state", new Date().toString(), ps.getStateInfo());
  }
  /**
  ...
  public void shouldIntegrateProgressiveRateCorrectly() 
    ...
    ps = new PayStationImpl( new ProgressiveRateStrategy() , new Revised(new Date().toString()));
