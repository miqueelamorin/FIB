...
int bfs(Matrixchar&m, int x, int y){
    ...
    int distmax = -1;
 	...
 	while (not Q.empty()) {
		...
		if (m[x][y] == 't') distmax = dist[x][y];
		m[x][y] = 'X';

		if (x != m.size()-1 and m[x+1][y] != 'X' and dist[x+1][y] == -1) {
            Q.push({x+1,y});
            dist[x+1][y] = dist[x][y] + 1;
        }

		if (x != 0 and m[x-1][y] != 'X' and dist[x-1][y] == -1) {
            Q.push({x-1,y});
            dist[x-1][y] = dist[x][y] + 1;
        }

		if (y != m[0].size()-1 and m[x][y+1] != 'X' and dist[x][y+1] == -1) {
            Q.push({x,y+1});
            dist[x][y+1] = dist[x][y] + 1;
        }

		if (y != 0 and m[x][y-1] != 'X' and dist[x][y-1] == -1) {
            Q.push({x,y-1});
            dist[x][y-1] = dist[x][y] + 1;
        }
...
int main (){
    ...
    if (d != -1) cout << "maximum distance: " << d << endl;
