...
void MyGLWidget::calcula_centre()
{
    ...
    escena_min = glm::vec3(-2.5, 0.0, -2.5);

    centre = (escena_max+escena_min)*0.5f;
...
void MyGLWidget::modelTransform(int n_patricio)
{
    // Matriu de transformació de model
    glm::mat4 transform (1.0f);
    ...
    if (n_patricio == 1) transform = glm::translate(transform, glm::vec3(2.f, 0.f, 2.f)); //moure a coords de cada patricio
    else if (n_patricio == 3) {
        transform = glm::translate(transform, glm::vec3(-2.f, 0.f, -2.f)); //moure a coords de cada patricio
        transform = glm::rotate(transform, float(M_PI), glm::vec3(0,1,0));
    }
    else {
        transform = glm::rotate(transform, float(M_PI*0.5), glm::vec3(0,1,0));
    }
...
{
    glm::mat4 View (1.f);
    //View = glm::lookAt(OBS, VRP, UP); //per definir matriu de camera

    View = glm::translate (View, glm::vec3(0.f,0.f,-2*radi));
    View = glm::rotate(View, theta, glm::vec3(1.f,0.f,0.f));       //rotem respecte a les x
    View = glm::rotate(View, -psi, glm::vec3(0.f,1.f,0.f));       //rotem respecte a les y
...
void MyGLWidget::mouseMoveEvent ( QMouseEvent * e )
{
    if (mouse_apretat){
        makeCurrent();
        psi += M_PI*(e->x() - old_x)/ample;
        theta -= M_PI*(e->y() - old_y)/alt;
        viewTransform();
        old_x = e->x();
        old_y = e->y();
        update();
